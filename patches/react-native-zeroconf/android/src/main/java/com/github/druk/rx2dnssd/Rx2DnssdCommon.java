...
abstract class Rx2DnssdCommon implements Rx2Dnssd {
    ...
    public Flowable<BonjourService> browse(@NonNull final String regType, @NonNull final String domain) {
        return browseOnInterface(regType, domain, DNSSD.ALL_INTERFACES);
    }

    @NonNull
    @Override
    public Flowable<BonjourService> browseOnInterface(
            @NonNull final String regType, @NonNull final String domain, final int ifIndex) {
        return createFlowable(
                emitter ->
                        mDNSSD.browse(
                                0, ifIndex, regType, domain, new Rx2BrowseListener(emitter)));
    }
